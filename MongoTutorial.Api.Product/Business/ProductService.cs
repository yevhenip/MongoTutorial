...
namespace MongoTutorial.Api.Product.Business
{
    public class ProductService : ServiceBase<Domain.Product>, IProductService
    {
        ...
        public async Task<Result<ProductDto>> GetAsync(string id)
        {
            ...
            if (cache.TryGetValue<Domain.Product>(out var cachedProduct))
            {
            ...
            productInDb = await FileExtensions.ReadFromFileAsync<Domain.Product>(_path, cacheKey + ".json");
        ...
        private async Task<Domain.Product> GetManufacturersAndUser(ProductModelDto product,
            IReadOnlyCollection<string> manufacturerIds)
        {
            ...
            var productToDb = Mapper.Map<Domain.Product>(product);
