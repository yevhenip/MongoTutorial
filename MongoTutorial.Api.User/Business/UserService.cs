...
namespace MongoTutorial.Api.User.Business
{
    public class UserService : ServiceBase<Domain.User>, IUserService
    {
        ...
        public async Task<Result<UserDto>> GetAsync(string id)
        {
            ...
            UserDto user;
            if (cache.TryGetValue<Domain.User>(out var cachedUser))
            {
            ...
            userInDb = await FileExtensions.ReadFromFileAsync<Domain.User>(_path, cacheKey + ".json");
        ...
        public async Task<Result<UserDto>> CreateAsync(UserDto user)
        {
            ...
            var userToDb = Mapper.Map<Domain.User>(user);
